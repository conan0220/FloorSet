...
def main():
    ...
    if args.evaluate:
        ...
        print("\n" + "=" * 100)
        print(f"DETAILED RESULTS: {result.submission_name}")
        print("=" * 100)
        print(f"{'ID':>4}  {'Blocks':>6}  {'Feasible':>8}  {'Overlaps':>8}  {'AreaViol':>8}  "
              f"{'HPWL':>12}  {'HPWL_base':>12}  {'HPWL_gap':>9}  "
              f"{'BBox':>12}  {'BBox_base':>12}  {'Area_gap':>9}  {'Cost':>8}  {'Time':>6}")
        print("-" * 100)
        for r in result.test_results:
            if r.error:
                print(f"{r.test_id:>4}  {'ERROR':>6}  {r.error[:60]}")
            else:
                feasible_str = "YES" if r.is_feasible else "NO"
                print(f"{r.test_id:>4}  {r.block_count:>6}  {feasible_str:>8}  "
                      f"{r.overlap_violations:>8}  {r.area_violations:>8}  "
                      f"{r.hpwl_total:>12.2f}  {r.hpwl_baseline:>12.2f}  {r.hpwl_gap:>+9.4f}  "
                      f"{r.bbox_area:>12.2f}  {r.area_baseline:>12.2f}  {r.area_gap:>+9.4f}  "
                      f"{r.cost:>8.4f}  {r.runtime_seconds:>5.2f}s")
                if args.log_blocks and r.positions:
                    print(f"       {'Block':>5}  {'x':>12}  {'y':>12}  {'w':>12}  {'h':>12}  {'area':>12}")
                    for bi, (x, y, w, h) in enumerate(r.positions):
                        print(f"       {bi:>5}  {x:>12.6f}  {y:>12.6f}  {w:>12.6f}  {h:>12.6f}  {w*h:>12.6f}")
        print("=" * 100)
